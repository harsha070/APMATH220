import sys
sys.path.append("/Users/harsha/Documents/courses/am220/PyGDebias")
sys.path.append("/Users/harsha/Documents/courses/am220/AIF360/")
...
class GNNExplainer(torch.nn.Module):
    ...
    def __loss__(self, pred, pred_perturb, mask_0=None, mask_1=None):

        # loss = torch.norm(torch.mean(pred[mask_0]) - torch.mean(pred_perturb[mask_0])) + torch.norm(torch.mean(pred[mask_1]) - torch.mean(pred_perturb[mask_1]))

        loss = torch.norm(pred - pred_perturb, 1)

        return loss

    def explain_graph(self, x, edge_index, perturbed_edge_index, mask_0=None, mask_1=None, **kwself):
        ...
        for e in range(0, 10):
            ...
            loss = self.__loss__(out, out_p, mask_0=mask_0, mask_1=mask_1)
            loss.backward()
            optimizer.step()
...
class fair_edit_trainer():
    def __init__(self, model=None, dataset=None, optimizer=None, features=None, edge_index=None,
                 labels=None, device=None, train_idx=None, val_idx=None, sens_idx=None, edit_num=4, sens=None,test_idx=None):
        self.model = model
        ...
        self.edge_weight = torch.ones(self.edge_index.shape[1], requires_grad=True)
        self.labels = labels
        self.device = device
    ...
    def add_drop_edge_random(self, add_prob=0.01, del_prob=0.1):
        ...
        if E < edits_to_make:
            edits_to_make = E
        top_bias_edge_indices = torch.topk(self.bias_edge_grads, k=edits_to_make).indices
        top_faithful_edge_indices = torch.topk(self.faithful_edge_grads, k=edits_to_make).indices
        top_delete_edge_indices = top_bias_edge_indices[~torch.isin(top_bias_edge_indices, top_faithful_edge_indices)]
        delete_indices = self.edge_index[:, top_delete_edge_indices]
    ...
    def fair_graph_edit(self):
        ...
        # print(f"deleted_edges: {deleted_edges}, added_edges: {added_edges}")
        ...
        # print(f"del_indices: {del_indices}, add_indices: {add_indices}")
        # generate gradients on these perturbations
        # TODO: review these changes
        mask_0 = torch.where(self.sens == 0)[0]
        mask_1 = torch.where(self.sens == 1)[0]
        edge_mask, perturbed_mask = grad_gen.explain_graph(self.features, self.edge_index, self.perturbed_edge_index, mask_0=mask_0, mask_1=mask_1)
        ...
        edits_per_epoch = 100
        best_add_indices = torch.topk(added_grads, edits_per_epoch, largest=False).indices
        best_del_indices = torch.topk(deleted_grads, edits_per_epoch, largest=False).indices

        # add edges
        best_add_edges = added_edges[:, best_add_indices]
        best_add_comp_edges = added_edges[:, best_add_indices]
        best_add_comp_edges[[0, 1]] = best_add_comp_edges[[1, 0]]
        self.edge_index = torch.cat((self.edge_index, best_add_edges, best_add_comp_edges), axis=1)

        # delete edges
        del_mask = torch.zeros(self.edge_index.shape[1])
        del_mask[best_del_indices] = 1
        self.edge_index = self.edge_index[:, del_mask == 0]
    ...
    def train(self, epochs=200):
        ...
        for epoch in range(epochs):
            ...
            output = self.model(self.features, self.edge_index, self.edge_weight)
            ...
            # faithful edge gradients
            self.faithful_edge_grads = self.edge_weight.grad

            self.optimizer.zero_grad()
            self.edge_weight = torch.ones(self.edge_index.shape[1], requires_grad=True)
            output = self.model(self.features, self.edge_index, self.edge_weight)
            # output = self.model(self.features, self.edge_index)
            mask_0 = torch.where(self.sens[self.train_idx] == 0)[0]
            mask_1 = torch.where(self.sens[self.train_idx] == 1)[0]
            loss_fair = torch.norm(torch.mean(output[self.train_idx][mask_0]) - torch.mean(output[self.train_idx][mask_1]))
            loss_fair.backward()
            # bias edge gradients
            self.bias_edge_grads = self.edge_weight.grad
            # loss_train = loss_train + 0.2 * loss_fair
            # loss_train.backward()
            # self.optimizer.step()
            ...
            #     }
            #     import pickle
            #     pickle.dump(res, open(f"fairedit_train_recidivism_tweak.pkl", 'wb'))
            ...
            if loss_val.item() < best_loss:
                ...
                arch = "gcn"
                model_type = "edge_edit"
                file_name = f"results/{dataset_name}_{arch}_{model_type}.pkl"
                result = {
                    "output": output,
                    "sens": self.sens,
                    "labels": self.labels,
        ...
        print("auc: {} f1: {} acc: {} parity: {} equality: {}".format(auc_roc_test, f1_s ,acc_s,parity_s, equality_s))
...
class GCN(nn.Module):
    ...
    def forward(self, x, edge_index, edge_weight=None):
        x = self.body1(x, edge_index, edge_weight)
        x = self.body2(x, edge_index, edge_weight)
        x = self.fc(x)
        return x
...
class FairFastEdit():
    def fit(self, adj, features, labels, idx_train, idx_val, idx_test, sens, sens_idx, model_name='gcn', epochs=500, lr=1e-3, weight_decay=5e-4, hidden=16, dropout=0.5):
        ...
        from scipy.sparse import coo_matrix
        edge_index = convert.from_scipy_sparse_matrix(coo_matrix(adj.to_dense()))[0]
