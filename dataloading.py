...
def process_synthetic(return_tensor_sparse=True, alpha=0.01, beta=0.1):
    import torch
    import numpy as np
    import random

    # num nodes
    num_nodes_per_dist = 1000

    # Generate data from two Gaussian distributions in 3D
    mu1 = np.array([0, 0, 0])
    mu2 = np.array([2, 2, 2])
    cov = np.eye(3)
    data1 = np.random.multivariate_normal(mu1, cov, size=num_nodes_per_dist)
    data2 = np.random.multivariate_normal(mu2, cov, size=num_nodes_per_dist)

    # Add the Gaussian distribution ID (0 or 1) to the features
    features1 = np.concatenate((data1, np.zeros((num_nodes_per_dist, 1))), axis=1)
    features2 = np.concatenate((data2, np.ones((num_nodes_per_dist, 1))), axis=1)
    features = np.concatenate((features1, features2), axis=0)
    sens = torch.FloatTensor(np.concatenate([np.zeros((num_nodes_per_dist, )), np.ones((num_nodes_per_dist, ))]))
    sens_idx = features.shape[1] - 1

    # Create a graph using the coordinates, the Gaussian distribution ID, and the label as node features
    n_nodes = features.shape[0]
    features = torch.tensor(features, dtype=torch.float)
    adjacency = torch.zeros((n_nodes, n_nodes), dtype=torch.float)
    # Create a label vector with 0 or 1 assigned at random
    labels = torch.tensor([random.randint(0, 1) for _ in range(n_nodes)], dtype=torch.float)

    # Add edges between points from different distributions with probability 0.01
    for i in range(n_nodes):
        for j in range(i+1, n_nodes):
            if i != j:
                if features[i][-2] != features[j][-2]:
                    if random.random() < alpha:
                        adjacency[i][j] = 1
                        adjacency[j][i] = 1

    # Add edges within points from the same distribution with probability 0.1
    for i in range(n_nodes):
        for j in range(i+1, n_nodes):
            if i != j:
                if features[i][-2] == features[j][-2]:
    ...
    idx_train = np.random.choice(list(range(n_nodes)), int(0.8 * n_nodes), replace=False)
    idx_val = list(set(list(range(n_nodes))) - set(idx_train))
    idx_test = np.random.choice(idx_val, len(idx_val) // 2, replace=False)
    idx_val = list(set(idx_val) - set(idx_test))
    ...
    return adjacency, features, labels, idx_train, idx_val, idx_test, sens, sens_idx
...
def load_data(dataset_name,  return_tensor_sparse=True):
    ...
    if dataset_name == "synthetic":
        return process_synthetic(return_tensor_sparse=return_tensor_sparse)
